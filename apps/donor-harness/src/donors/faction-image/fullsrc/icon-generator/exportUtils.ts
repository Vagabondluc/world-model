...
export function toSVGString(spec: IconSpec): string {
  ...
  if (filters.length || needsBlendFallback) {
    ...
    if (needsBlendFallback) {
      const blendModes = Array.from(new Set(spec.layers.map((layer) => layer.blendMode).filter((m): m is import("./types").BlendMode => m != null && m !== "normal")));
      for (const mode of blendModes) {
        ...
        fe.setAttribute("mode", String(mode));
