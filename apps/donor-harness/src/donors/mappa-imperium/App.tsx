...
import { subscribeCanonicalToGameStore } from '@mi/store/canonicalBridge';
...
// Wire canonical store → gameStore so world data from canonical is reflected in game state
function CanonicalSync() {
    useEffect(() => {
        const { setMapData, elements } = useGameStore.getState();
        const unsubscribe = subscribeCanonicalToGameStore(
            (mapData) => setMapData(mapData as Parameters<typeof setMapData>[0]),
            (newElements) => {
                // Bulk-replace elements in the game store when canonical changes
                useGameStore.setState({ elements: newElements as typeof elements });
            },
        );
        return unsubscribe;
    }, []);
...
const App = () => {
    ...
    return (
        <>
            {/* Syncs canonical store world data into the gameStore */}
            <CanonicalSync />
