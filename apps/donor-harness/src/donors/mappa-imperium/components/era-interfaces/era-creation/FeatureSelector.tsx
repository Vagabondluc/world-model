...
import type { GeographyType, EraCreationContextType } from '@mi/types';
...
const FeatureSelector = ({ state }: FeatureSelectorProps) => {
    const { landmassType, features, setState, handleFeatureChange } = state;
    const landmassOptions = getLandmassOptions(landmassType);

    return (
        <>
            <div>
                <label htmlFor="landmassType" className={componentStyles.form.label}>
