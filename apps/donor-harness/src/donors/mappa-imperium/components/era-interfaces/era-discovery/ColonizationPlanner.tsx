...
import type { Player, ElementCard, Character, GameSettings } from '@mi/types';
            ...
            <form onSubmit={handleSubmit} className="p-6 border rounded-lg bg-white shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div className="col-span-2">
                        <label htmlFor="hero-name" className={componentStyles.form.label}>Hero's Name</label>
                        <input type="text" id="hero-name" value={name} onChange={e => setName(e.target.value)} required className={componentStyles.input.base} disabled={isComplete} />
                    </div>
                    <div>
                        <label htmlFor="hero-year" className={componentStyles.form.label}>Year of Achievement</label>
                        <input type="number" id="hero-year" value={year} onChange={e => handleYearChange(e.target.value)} min={era4YearRange.start} max={era4YearRange.end} required className={componentStyles.input.base} disabled={isComplete} />
                        {yearError && <p className="text-xs text-red-600 mt-1">{yearError}</p>}
                    </div>
                </div>
                 <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label htmlFor="hero-profession" className={componentStyles.form.label}>Profession (Rule 4.2)</label>
                        <select id="hero-profession" value={profession} onChange={e => setProfession(e.target.value)} className={componentStyles.input.base} disabled={isComplete}>
                            {professions.map(p => <option key={p} value={p}>{p}</option>)}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="hero-achievement" className={componentStyles.form.label}>Achievement Type</label>
                        <select id="hero-achievement" value={achievementType} onChange={e => setAchievementType(e.target.value)} className={componentStyles.input.base} disabled={isComplete}>
                            {achievementTypes.map(p => <option key={p} value={p}>{p}</option>)}
                        </select>
                    </div>
                </div>
                 <div>
                    <label htmlFor="hero-status" className={componentStyles.form.label}>Current Status</label>
                    <select id="hero-status" value={status} onChange={e => setStatus(e.target.value)} className={componentStyles.input.base} disabled={isComplete}>
                        {statuses.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                </div>
                <div>
                    <label htmlFor="hero-description" className={componentStyles.form.label}>Specific Achievement / Legacy</label>
                    <textarea id="hero-description" value={description} onChange={e => setDescription(e.target.value)} rows={4} maxLength={1200} required className={cn(componentStyles.input.base, "resize-y")} disabled={isComplete} placeholder="Describe the act that made them a hero..."/>
                </div>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="location-to-honor" className={componentStyles.form.label}>Location to Honor</label>
                        <input type="text" id="location-to-honor" value={locationToHonor} onChange={e => setLocationToHonor(e.target.value)} className={componentStyles.input.base} disabled={isComplete} placeholder="e.g., The Northern Forest"/>
                    </div>
                     <div>
                        <label htmlFor="new-location-name" className={componentStyles.form.label}>New Honored Name</label>
                        <input type="text" id="new-location-name" value={newLocationName} onChange={e => setNewLocationName(e.target.value)} className={componentStyles.input.base} disabled={isComplete} placeholder="e.g., Elara's Wood"/>
                    </div>
                </div>
                <div className="flex gap-4 pt-4 border-t">
                    <button type="button" onClick={handleClear} className={cn(componentStyles.button.base, componentStyles.button.secondary, "w-1/3")}>Clear</button>
                    <button type="submit" disabled={isComplete} className={cn(componentStyles.button.base, componentStyles.button.primary, "w-2/3")}>Create Hero</button>
                </div>
