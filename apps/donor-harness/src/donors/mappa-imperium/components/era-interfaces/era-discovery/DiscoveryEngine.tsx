...
import type { Player, ElementCard, GameSettings, Resource, Location, Faction, Settlement, Event as EventType, Character } from '@mi/types';
    ...
    const renderCreationFormFields = () => {
        ...
        const nameField = (
            <div className="md:col-span-2">
                <label htmlFor="element-name" className={componentStyles.form.label}>Name</label>
                <input type="text" id="element-name" value={formState.name} onChange={e => setFormState(p => ({...p, name: e.target.value}))} required className={componentStyles.input.base} />
            </div>
        );
        ...
        const yearField = (
            <div>
                ...
                <input type="number" id="element-year" value={formState.year} onChange={e => handleYearChange(e.target.value)} min={yearRange.start} max={yearRange.end} required className={componentStyles.input.base} />
        ...
        const symbolField = (
             <div className="relative">
                <label className={componentStyles.form.label}>Map Emoji</label>
                <button ref={triggerRef} type="button" onClick={() => setIsPickerOpen(true)} className={cn(componentStyles.input.base, "text-2xl text-left")}>{formState.symbol}</button>
             </div>
        );

        const typeSelectField = (label: string, options: readonly string[]) => (
            <div>
                <label className={componentStyles.form.label}>{label}</label>
                <select value={formState.selectValue1} onChange={e => setFormState(p => ({...p, selectValue1: e.target.value}))} className={componentStyles.input.base}>
                ...
                <div className="pt-4 border-t">
                    {selectedEvent && (
                        <form onSubmit={handleCreateElement} className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="narrative-edit" className={componentStyles.form.label}>Description / Narrative</label>
                                <textarea id="narrative-edit" value={formState.description} onChange={e => setFormState(p => ({...p, description: e.target.value}))} rows={8} maxLength={1200} className={cn(componentStyles.input.base, "resize-y")} placeholder="Describe the event manually, or use the AI generator..." required/>
                            </div>
                            
                            <div className="flex gap-4 pt-4 border-t">
                                <button type="submit" disabled={!formState.name || typeof formState.year !== 'number' || !!yearError} className={cn(componentStyles.button.base, componentStyles.button.primary, "w-full")}>Create Element & Complete Turn</button>
                            </div>
