...
import type { ElementCard, Monument } from '@mi/types';
...
const MonumentCard = ({ element, onEdit, onDelete, onExportHtml, onExportMarkdown }: MonumentCardProps) => {
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const menuRef = useRef<HTMLDivElement>(null);
    useOnClickOutside(menuRef, () => setIsMenuOpen(false));

    if (element.type !== 'Monument') return null;
    const monumentData = element.data as Monument;

    const buttonId = `menu-button-monument-${element.id}`;
            ...
            <div className="flex gap-4 items-start">
                ...
                <div className="flex-grow">
                     <div className="flex justify-between items-start">
                        <h4 className="text-xl font-bold text-gray-800">{monumentData.name}</h4>
                        <div className="relative" ref={menuRef}>
                            <button id={buttonId} onClick={() => setIsMenuOpen(!isMenuOpen)} className="btn-menu-trigger" aria-haspopup="true" aria-expanded={isMenuOpen} aria-controls={`menu-monument-${element.id}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" /></svg>
                            </button>
                            {isMenuOpen && (
                                <div id={`menu-monument-${element.id}`} className="dropdown-menu w-48 animate-fade-in-down" role="menu" aria-orientation="vertical" aria-labelledby={buttonId}>
                    ...
                    <p className="mt-1 text-gray-600 whitespace-pre-wrap text-sm">{monumentData.description}</p>
                </div>
            </div>
        </div>
    );
