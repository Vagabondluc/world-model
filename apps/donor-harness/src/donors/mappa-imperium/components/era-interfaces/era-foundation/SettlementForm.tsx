...
import type { Settlement, Faction } from '@mi/types';
    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 border rounded-lg bg-white shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div>
                    <label htmlFor="settlement-name" className={componentStyles.form.label}>Settlement Name</label>
                    <input type="text" id="settlement-name" value={name} onChange={e => setName(e.target.value)} required className={componentStyles.input.base} disabled={disabled}/>
                </div>
                <div>
                    <label htmlFor="settlement-purpose" className={componentStyles.form.label}>Purpose</label>
                    <select id="settlement-purpose" value={purpose} onChange={e => setPurpose(e.target.value)} className={componentStyles.input.base} disabled={disabled || !!preselectedPurpose}>
                        {settlementPurposes.filter(p => p !== 'Capital').map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                </div>
            </div>
            <div>
                <label htmlFor="settlement-description" className={componentStyles.form.label}>Description</label>
                <textarea id="settlement-description" value={description} onChange={e => setDescription(e.target.value)} rows={3} required className={componentStyles.input.base} disabled={disabled} />
            </div>
