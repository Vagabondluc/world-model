...
import { EraStorageService } from '@mi/services/eraStorageService';
import { ERA_TEMPLATES } from '@mi/data/era-templates';
import type {
    NodeDefinition,
    ...
    ConnectionId,
    GraphError
} from '@mi/types/nodeEditor.types';
...
export const createElementEditorSlice: GameSliceCreator<ElementEditorSlice> = (set, get) => ({
    ...
    saveCurrentEra: async (name: string) => {
        const state = get();
        const { nodeEditorEraId, nodes, connections } = state;

        if (nodeEditorEraId === null) {
        ...
        const schema: NodeEditorSchema = {
            ...
            metadata: {
                name,
                description: `Saved graph for Era ${nodeEditorEraId}`,
                createdAt: new Date().toISOString(),
                modifiedAt: new Date().toISOString(),
            },
            nodes,
        ...
        try {
            await EraStorageService.saveEraGraph(nodeEditorEraId, name, schema);
    ...
    loadSavedEra: async (savedId: string) => {
        try {
            ...
            if (saved && saved.schema) {
                ...
                // set({ nodeEditorEraId: saved.eraId }); 
            }
        } catch (error) {
    ...
    refreshSavedEras: async () => {
        const { nodeEditorEraId } = get();
        try {
            const saves = await EraStorageService.listSavedEras(nodeEditorEraId || undefined);
            set({ savedEras: saves });
        } catch (error) {
    ...
    loadReferenceTemplate: () => {
        const { nodeEditorEraId } = get();
        if (nodeEditorEraId && ERA_TEMPLATES[nodeEditorEraId]) {
            get().importSchema(ERA_TEMPLATES[nodeEditorEraId]);
        } else {
            console.warn(`No reference template found for era ${nodeEditorEraId}`);
        }
    },
    ...
    loadSampleProject: async () => {
        try {
            const { SAMPLE_PROJECT_DATA } = await import('@mi/data/sampleProject');
            await EraStorageService.importAllEras(SAMPLE_PROJECT_DATA.saves);
            await get().refreshSavedEras();

            // Auto-load the graph for the current era if available
            const state = get();
            const nodeEditorEraId = state.nodeEditorEraId || 1;
            const sampleParams = SAMPLE_PROJECT_DATA.saves.find(s => s.eraId === nodeEditorEraId);
