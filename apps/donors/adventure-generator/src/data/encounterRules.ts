...
// Based on DMG p.82
export const ENCOUNTER_MULTIPLIERS: { monsters: number | [number, number]; multiplier: number }[] = [
    ...
    { monsters: 15, multiplier: 4 }, // And more
];
...
export function getEncounterMultiplier(monsterCount: number): number {
    ...
    for (const entry of ENCOUNTER_MULTIPLIERS) {
        if (typeof entry.monsters === 'number') {
            if (monsterCount === entry.monsters) return entry.multiplier;
        } else {
            if (monsterCount >= entry.monsters[0] && monsterCount <= entry.monsters[1]) {
                return entry.multiplier;
            }
        }
    }
    return 4; // For 15+ monsters
}