...
export type LayerAssociation = {
    associatedMapId: string | null;  // Which map layer this location belongs to
    isEchoed: boolean;              // Whether this is an "echo" (copy) from another layer
    originalLayerId?: string;       // If echoed, the original layer this was copied from
};

export type ManagedLocation = z.infer<typeof ManagedLocationSchema> & {
    layerAssociation?: LayerAssociation;  // Track layer relationships
};
