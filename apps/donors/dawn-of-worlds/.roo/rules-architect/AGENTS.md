...
- **Event-sourced architecture**: State is derived from events via `deriveWorld()` and `derivePlayerState()`. Design features around event dispatching, not state mutation.
- **Importmap dependencies**: All external dependencies are loaded via esm.sh CDN in `index.html`. Architecture must support browser-only runtime without bundling.
