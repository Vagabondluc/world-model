# SPEC-045: Smooth Spherical Globe Architecture

## Purpose

This specification defines architectural solution for smooth spherical globe geometry while maintaining compatibility with hex-based gameplay system. The design provides a pure smooth spherical rendering approach for the globe.
...
6. **Vertical Layering**: Support for altitude/depth zones and object stacking (see [Spec 047](047-vertical-layering-system.md))
...
**Normative**: The engine uses **Three.js default right-handed Y-up** coordinates.

- **Up axis = +Y**, forward = −Z, right = +X
- **Spherical coordinates**:
  - Latitude φ ∈ [-π/2, +π/2] (south → north)
  - Longitude λ ∈ (-π, +π] (west → east)

**Conversion** (spherical to Cartesian):

```typescript
function sphericalToCartesian(latitude: number, longitude: number, radius: number): Vec3 {
    const phi = latitude;
...
```typescript
function cartesianToSpherical(pos: Vec3): { latitude: number; longitude: number; radius: number } {
    const radius = Math.sqrt(pos.x * pos.x + pos.y * pos.y + pos.z * pos.z);
    const latitude = Math.asin(pos.y / radius);
    const longitude = Math.atan2(pos.z, pos.x);
    
    return { latitude, longitude, radius };
}
```

All sample code in this specification uses Y as the vertical axis.

### 0.5 Key Decoupling Points
...
###1.6 Two Meshes and the Dual Relationship

**Explicit Statement**: The smooth spherical globe architecture generates **two related structures**:

1. **Render Mesh**: A subdivided **icosahedron** (icosphere topology) composed of triangles
2. **Cell Mesh**: The **dual** of the render mesh, composed of hexagons (majority) and exactly 12 pentagons

**Terminology Clarification**:

- "Goldberg polyhedron" refers to the hex/pentagon tiling on a sphere produced as the **dual of a triangulated sphere**
- We do NOT generate a separate parametric Goldberg polyhedron first
- We derive the cell mesh (Goldberg-like) from the triangulation's dual

**Relationship**:

- Each **vertex** of the render mesh (icosphere) becomes a **cell center** in the cell mesh
- Each **face** (triangle) of the render mesh defines **adjacency** between three cells
- The **edges** of a cell are formed by connecting the centers of adjacent triangular faces (circumcenters)

This dual relationship ensures:
- Exactly 12 pentagons (corresponding to the 12 vertices of the base icosahedron)
- All other cells are hexagons
- Uniform distribution across the sphere
- Clean topological properties

### 1.7 Dual Mesh Extraction Algorithm

**Input**: Render mesh consisting of:
- Vertex list `V` (positions on sphere)
- Face list `F`, where each face is a triple of vertex indices `(a, b, c)`
- Adjacency information: for each vertex, the incident faces in cyclic order

**Output**: Cell mesh where each cell has:
- `id`: Cell identifier (vertex index from render mesh)
- `center`: Normalized position of the corresponding vertex
- `corners`: Ordered list of vertices forming the cell polygon

**Algorithm** (normative pseudocode):

```typescript
interface DualMeshConfig {
    vertices: Vec3[];
    faces: number[][];  // [a, b, c] vertex indices
}

function buildDualCells(mesh: DualMeshConfig): Cell[] {
    const { vertices, faces } = mesh;
    
    // Step 1: Precompute circumcenters for each face
    const faceCircumcenters: Vec3[] = faces.map(([a, b, c]) => 
        sphericalCircumcenter(vertices[a], vertices[b], vertices[c])
    );
    
    // Step 2: Build incidence list - which faces touch each vertex
    const incidentFaces: Map<number, number[]> = new Map();
    
    for (let faceIdx = 0; faceIdx < faces.length; faceIdx++) {
        const [a, b, c] = faces[faceIdx];
        
        if (!incidentFaces.has(a)) incidentFaces.set(a, []);
        if (!incidentFaces.has(b)) incidentFaces.set(b, []);
        if (!incidentFaces.has(c)) incidentFaces.set(c, []);
        
        incidentFaces.get(a)!.push(faceIdx);
        incidentFaces.get(b)!.push(faceIdx);
        incidentFaces.get(c)!.push(faceIdx);
    }
    
    // Step 3: For each vertex, create a cell
    const cells: Cell[] = [];
    
    for (let vertexIdx = 0; vertexIdx < vertices.length; vertexIdx++) {
        const faceIndices = incidentFaces.get(vertexIdx) || [];
        
        // Sort faces cyclically around this vertex
        const orderedFaces = sortFacesAroundVertex(
            vertexIdx,
            faceIndices,
            vertices,
            faceCircumcenters
        ...
        // Cell polygon = ordered circumcenters
        const corners = orderedFaces.map(fIdx => faceCircumcenters[fIdx]);
        
        cells.push({
            id: `cell-${vertexIdx}`,
            kind: corners.length === 5 ? 'PENT' : 'HEX',
            center: vec3.normalize(vertices[vertexIdx]),
            corners,
            neighbors: [], // Derived from adjacency in next step
            face: 0,  // Base icosahedron face (for compatibility)
            local: { x: 0, y: 0 }
        });
    }
...
1. **Spherical Circumcenter**:

```typescript
function sphericalCircumcenter(p: Vec3, q: Vec3, r: Vec3): Vec3 {
    // Compute the circumcenter of triangle (p,q,r) on the sphere
    // Method: Find intersection of perpendicular bisector planes
    
    const pq = vec3.normalize(vec3.cross(p, q));
    const qr = vec3.normalize(vec3.cross(q, r));
    
    // Circumcenter direction
    const cc = vec3.normalize(vec3.cross(pq, qr));
    
    // Choose correct direction (towards triangle)
    const midpoint = vec3.normalize(vec3.add(vec3.add(p, q), r));
    if (vec3.dot(cc, midpoint) < 0) {
        return vec3.scale(cc, -1);
    }
...
2. **Sort Faces Around Vertex**:

```typescript
function sortFacesAroundVertex(
    vertexIdx: number,
    faceIndices: number[],
    vertices: Vec3[],
    facecenters: Vec3[]
): number[] {
    const normal = vec3.normalize(vertices[vertexIdx]);
    
    // Create tangent basis around vertex
    const arbitrary = Math.abs(normal.y) < 0.9 
        ? { x: 0, y: 1, z: 0 } 
        : { x: 1, y: 0, z: 0 };
    
    const tangentU = vec3.normalize(vec3.cross(normal, arbitrary));
    const tangentV = vec3.cross(normal, tangentU);
    
    // Compute angle for each face center
    const faceAngles: { idx: number; angle: number }[] = faceIndices.map(fIdx => {
        const center = facecenters[fIdx];
        
        // Project center into tangent plane
        const offset = vec3.subtract(center, vec3.scale(normal, vec3.dot(center, normal)));
        const u = vec3.normalize(offset);
        
        // Compute angle
        const angleU = vec3.dot(u, tangentU);
        const angleV = vec3.dot(u, tangentV);
        const angle = Math.atan2(angleV, angleU);
        
        return { idx: fIdx, angle };
    });
    
    // Sort by angle
    faceAngles.sort((a, b) => a.angle - b.angle);
    
    return faceAngles.map(fa => fa.idx);
}
```

**Pentagon Identification**:

In a subdivided icosphere:
- Vertex valence is **5** for the 12 original icosahedron vertices
- Vertex valence is **6** for all vertices created during subdivision

Therefore:
- **Pentagon cells are exactly those with 5 corners** (5 incident faces)
- **Hexagon cells have 6 corners** (6 incident faces)

This automatically satisfies the requirement of exactly 12 pentagons.

### 1.8 Pentagon Placement

**Normative**: Pentagons are fixed at the **12 base icosahedron vertices** after subdivision.

**Implementation Requirements**:

1. **Identify Base Vertices**: Before subdivision, mark the 12 base icosahedron vertices
2. **Persist Marking**: During subdivision, ensure base vertices retain their identity
3. **Validation**: After dual mesh extraction:
   - `count(cells where kind === 'PENT') MUST equal 12`
   - Those 12 cell centers MUST correspond to base icosahedron vertex positions (within epsilon = 1e-6)
...
interface Vertex {
    position: Vec3;
    originTag: 'BASE' | 'EDGE' | 'FACE';
}

function markBaseVertices(vertices: Vertex[]): void {
    // First 12 vertices are always base icosahedron vertices
    for (let i = 0; i < 12; i++) {
        vertices[i].originTag = 'BASE';
    }
}

function validatePentagons(cells: Cell[], baseVertices: Vertex[]): void {
    const pentagons = cells.filter(c => c.kind === 'PENT');
    
    if (pentagons.length !== 12) {
        throw new Error(`Expected 12 pentagons, got ${pentagons.length}`);
    }
    
    // Verify each pentagon corresponds to a base vertex
    const basePositions = baseVertices
        .filter(v => v.originTag === 'BASE')
        .map(v => v.position);
    
    for (const pentagon of pentagons) {
        const matchFound = basePositions.some(basePos => 
            vec3.distance(pentagon.center, basePos) < 1e-6
        );
        
        if (!matchFound) {
            throw new Error(`Pentagon at ${pentagon.center} does not match base vertex`);
        }
    }
}
```

This deterministic placement ensures:
- Predictable pentagon locations
- Consistent topology across different subdivision levels
...
The coordinate system maintains a three-layer approach:
...
gantt
    ...
    section Phase 1
    Smooth Sphere Mesh     :p1-1, 2026-02-01, 7d
...
The globe specification ([`docs/specs/new-specs/globe.md`](new-specs/globe.md)) should reference this smooth spherical architecture:

1. **Use smooth sphere mesh** as base geometry
...
The proposed dual-layer architecture provides a clean separation between logical hex grid (gameplay) and visual smooth sphere (rendering). This approach:
...
The architecture is compatible with the browser-only, no-build constraint and respects the importmap in [`index.html`](index.html). Three.js can be added via CDN as specified.
