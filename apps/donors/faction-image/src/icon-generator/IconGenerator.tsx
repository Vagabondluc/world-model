...
function applyOverlayLayers(
  ...
  const toTransform = (layer: LayerItem) => {
    ...
    return `translate(${tx} ${ty}) translate(${ox} ${oy}) rotate(${rot}) scale(${baseScale * layer.content.scale * sx} ${baseScale * layer.content.scale * sy}) translate(${-64} ${-64})`;
  };
  ...
  const symbolOverlays: Layer[] = renderable
    ...
    .map((layer) => ({
      ...
      d: SYMBOL_PATHS[layer.content.symbol],
  ...
  useEffect(() => {
    ...
    const queue = async () => {
      const needsRefresh = assetLayers.filter((layer) => {
        ...
        const signature = `${layer.content.assetPath}|${JSON.stringify(layer.content.recolor)}`;
      ...
      await Promise.all(needsRefresh.map(async (layer) => {
        ...
        await recolorQueueRef.current.enqueue(async () => {
          try {
            const raw = await assetProviderRef.current.loadRawSvg(layer.content.assetPath);
            const recolored = recolorEngineRef.current.recolor(raw, layer.content.recolor);
            if (cancelled) return;
            setAssetRenderByLayerId((prev) => ({
              ...prev,
              [layer.layerId]: {
