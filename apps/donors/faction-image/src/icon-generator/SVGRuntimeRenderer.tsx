...
import type { FilterDef, IconSpec, Layer } from "./types";
...
export function SVGRuntimeRenderer({
  ...
  const fallbackBlendModes = Array.from(new Set(spec.layers.map((layer) => layer.blendMode).filter((m): m is string => !!m && m !== "normal")));
  return (
    <svg
