...
const GeographyAdvicePanel = () => {
    const { landmassType, features, advice, isLoading, error, getAIAdvice, userInputAdvice, setState } = useEraCreation();
    const { elements } = useAI();
        ...
        <div className="p-6 border rounded-lg bg-white shadow-sm space-y-6">
             <AIGenerationSection title="Holistic Placement Advisor">
                ...
                <button
                    ...
                    className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition-all transform hover:scale-105 shadow-lg disabled:bg-gray-400 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
                >
