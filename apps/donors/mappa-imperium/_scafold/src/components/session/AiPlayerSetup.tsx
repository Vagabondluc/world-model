...
import type { AiPlayerSetupProps, Player, AIPersonality } from '../../types';
...
const AiPlayerSetup = ({ gameSettings, onComplete, onBack }: AiPlayerSetupProps) => {
...
- "biography": A short, 2-3 sentence narrative backstory for the character.`;
    ...
    const handleFinalize = () => onComplete(aiPlayers);
        ...
        <div className="layout-centered-card p-4">
             <div className="page-card max-w-4xl">
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-6">
                    <div>
                        <label htmlFor="ai-select" className="form-label">Configure Player</label>
                        <select id="ai-select" value={selectedPlayerIndex} onChange={e => setSelectedPlayerIndex(Number(e.target.value))} className="input-base">
                            {aiPlayers.map((p, index) => <option key={p.playerNumber} value={index}>AI Player {p.playerNumber}{p.name ? ` (${p.name})` : ''}</option>)}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="profile-select" className="form-label">Load Profile</label>
                        <select id="profile-select" onChange={handleProfileLoad} className="input-base" defaultValue="">
                ...
                <div className="p-6 border rounded-lg bg-gray-50/50 shadow-inner space-y-4 max-h-[60vh] overflow-y-auto pr-4">
                    <div className="grid md:grid-cols-2 gap-x-6 items-start">
                        <div>
                            <label htmlFor={`ai-name-${selectedPlayer.playerNumber}`} className="form-label">Custom Name</label>
                            <input id={`ai-name-${selectedPlayer.playerNumber}`} type="text" value={selectedPlayer.name} onChange={(e) => handlePlayerPropertyChange(selectedPlayerIndex, { name: e.target.value })} className="input-base text-xl" placeholder={`Enter name for AI Player ${selectedPlayer.playerNumber}`} />
                        </div>
                        <div>
                            <label htmlFor={`ai-playstyle-${selectedPlayer.playerNumber}`} className="form-label">Play Style</label>
                            <select
                    ...
                    <details className="group pt-4">
                        ...
                        <div className="mt-4 pt-4 border-t space-y-4">
                             <div>
                                <label htmlFor={`ai-persona-${selectedPlayer.playerNumber}`} className="form-label">Persona Summary</label>
                                <textarea id={`ai-persona-${selectedPlayer.playerNumber}`} value={personality.persona} onChange={(e) => handlePersonalityChange(selectedPlayerIndex, { persona: e.target.value })} rows={2} className="input-base" placeholder="A one-sentence directive for the AI." />
                            </div>
                            <div>
                                <label htmlFor={`ai-bio-${selectedPlayer.playerNumber}`} className="form-label">Biography</label>
                                <textarea id={`ai-bio-${selectedPlayer.playerNumber}`} value={personality.biography} onChange={(e) => handlePersonalityChange(selectedPlayerIndex, { biography: e.target.value })} rows={3} className="input-base" placeholder="A short backstory." />
                            </div>
                            <AIGenerationSection title="AI-Powered Generation">
                                <AIContextInput id={`ai-generation-input-${selectedPlayer.playerNumber}`} label="Inject Ideas into AI Generation" value={generationInputs[selectedPlayer.playerNumber] || ''} onChange={(val) => setGenerationInputs(prev => ({...prev, [selectedPlayer.playerNumber]: val }))} elements={elements} placeholder="e.g., 'themed around fire', 'from a lost lineage'..." rows={2} />
                 ...
                 <div className="mt-8 pt-4 border-t flex items-center gap-4">
                     <button onClick={onBack} className="w-1/3 btn btn-secondary">&larr; Back to Setup</button>
                    <button onClick={handleFinalize} className="w-2/3 btn btn-primary disabled:bg-gray-400 disabled:cursor-not-allowed" disabled={!isConfigurationComplete} title={!isConfigurationComplete ? 'All AI players must have a name.' : 'Proceed to player selection'}>
