...
const ChronicleLobby = ({ onImportFromFeed, onBackToSetup }: ChronicleLobbyProps) => {
                ...
                {games.map(game => (
                    <div key={game.gameId} className="bg-white p-4 rounded-lg shadow-sm border flex flex-col md:flex-row items-start md:items-center gap-4">
                        ...
                        <div className="w-full md:w-auto text-left md:text-right">
                            ...
                            <button 
                                onClick={() => onImportFromFeed(game.feedUrl)}
                                className="btn btn-primary bg-sky-700 hover:bg-sky-600 w-full mt-2"
                            >
                                Load as Observer
                            </button>
                        </div>
        ...
        <div className="layout-centered-card p-4">
            <div className="page-card max-w-2xl">
                ...
                <div className="mt-8 pt-4 border-t">
                    <button onClick={onBackToSetup} className="w-full btn btn-secondary">
                        &larr; Back to Setup
                    </button>
                </div>
            </div>
