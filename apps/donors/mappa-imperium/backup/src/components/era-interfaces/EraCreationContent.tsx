        ...
        return (
            <>
                <StepProgressBar 
                    ...
                    onStepChange={(stepId) => updateEraUiState({ eraOne: { gameplayStep: stepId as 'geography' | 'resources' }})}
                />
                <div className="space-y-8">{renderCurrentStep()}</div>
            </>
        );
    }, [gameplayStep, updateEraUiState, sidebarElements.length, stepProgress, eraCreationState]);
