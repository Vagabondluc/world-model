...
import type { Player, ElementCard, GameSettings, Location } from '../../../types';
...
const IconicLandmarkCreator = ({ currentPlayer, elements, onCreateElement, gameSettings }: IconicLandmarkCreatorProps) => {
    ...
    const { result, isLoading, error, generate, clear } = useAIGeneration();
    ...
    return (
        <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="p-6 border rounded-lg bg-white shadow-sm space-y-4">
                <div>
                    <label htmlFor="landmark-name" className={componentStyles.form.label}>Landmark Name</label>
                    <input type="text" id="landmark-name" value={formState.name} onChange={e => setFormState(p => ({...p, name: e.target.value}))} required className={componentStyles.input.base} />
                </div>
                 <div>
                    <label htmlFor="landmark-desc" className={componentStyles.form.label}>Historical Significance</label>
                    <textarea id="landmark-desc" value={formState.description} onChange={e => setFormState(p => ({...p, description: e.target.value}))} required rows={8} className={componentStyles.input.base} placeholder="Describe the key events and people that made this place important..." />
                </div>

                <AIGenerationSection title="AI-Powered Generation">
                    <button type="button" onClick={handleGenerate} disabled={isLoading || !formState.name} className={cn(componentStyles.button.base, componentStyles.button.primary, "w-full bg-blue-600 hover:bg-blue-700")}>
                ...
                <div className="pt-4 border-t">
                    <button type="submit" disabled={!formState.name || !formState.description} className={cn(componentStyles.button.base, componentStyles.button.primary, "w-full")}>
