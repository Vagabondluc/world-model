...
const SystemInfoTab: React.FC<SystemInfoTabProps> = ({ systemInfo }) => {
    return (
        <div className="space-y-4">
            ...
            <div className="bg-white border rounded-lg p-4">
                <h4 className="font-semibold mb-3 text-gray-900">Network & Platform</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-900">
                    <div><strong className="block">Connection Type:</strong> {systemInfo?.connection?.effectiveType || 'N/A'}</div>
                    <div><strong className="block">Downlink:</strong> {systemInfo?.connection?.downlink ? `${systemInfo.connection.downlink} Mbps` : 'N/A'}</div>
                    <div><strong className="block">RTT:</strong> {systemInfo?.connection?.rtt ? `${systemInfo.connection.rtt} ms` : 'N/A'}</div>
                </div>
            </div>
        </div>
    );
