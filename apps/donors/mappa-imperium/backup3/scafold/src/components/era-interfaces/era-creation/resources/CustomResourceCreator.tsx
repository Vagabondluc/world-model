...
const CustomResourceCreator = ({ onCreate, disabled }: CustomResourceCreatorProps) => {
    ...
    // AI State
    const { landmassType, features } = useEraCreation();
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Validate and set symbol to ensure it's an emoji
            ...
            clear(); // Clear the result from context after processing
        } catch (e) {
    ...
    const handleAIGenerate = async () => {
        ...
        const geographySummary = features.filter(f => f.type).map(f => f.type).join(', ');
        let basePrompt = `For a fantasy world region that is a "${landmassType || 'diverse area'}" with features like ${geographySummary || 'various terrains'}, create a unique resource or special site.`;
        if (isMundane) {
            basePrompt = `For a fantasy world region that is a "${landmassType || 'diverse area'}" with features like ${geographySummary || 'various terrains'}, create a unique but MUNDANE (non-magical and not supernatural) resource or special site.`;
        }
    ...
    const isAiDisabled = isGenerating || disabled || properties.trim() !== '';
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                    <div className="col-span-2">
                        ...
                        <input type="text" id="resourceName" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Sunstone, Whisperwood" required className={inputClasses} />
                    </div>
                    <div className="relative">
                        <label htmlFor="resourceSymbol" className="block text-sm font-medium text-gray-700">Symbol</label>
                        <button ref={triggerRef} type="button" id="resourceSymbol" onClick={() => setIsPickerOpen(!isPickerOpen)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-center text-2xl bg-white" aria-haspopup="true" aria-expanded={isPickerOpen}>{symbol}</button>
                ...
                <div>
                    ...
                    <select id="resourceType" value={type} onChange={e => setType(e.target.value as Resource['type'])} className={inputClasses}>
                ...
                <div>
                    <label htmlFor="resourceProperties" className="block text-sm font-medium text-gray-700">Properties & Uniqueness</label>
                    <textarea id="resourceProperties" value={properties} onChange={e => setProperties(e.target.value)} rows={5} placeholder="Describe what makes it special..." required className={inputClasses} />
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t">
                    ...
                    <button type="submit" disabled={disabled} className="w-2/3 bg-amber-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-amber-700 transition-all shadow-lg disabled:bg-gray-400 disabled:cursor-not-allowed">
                ...
                <AIGenerationSection title="AI-Powered Generation">
                    ...
                    <button type="button" onClick={handleAIGenerate} disabled={isAiDisabled} className="w-full flex justify-center items-center gap-2 bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 transition-all shadow disabled:bg-gray-400 disabled:cursor-not-allowed">
                            {isGenerating ? <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> Generating...</> : 'Generate with AI'}
                    </button>
                    {properties.trim() !== '' && !isGenerating && (
