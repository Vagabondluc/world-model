    ...
    return (
        <form onSubmit={handleSubmit} className="p-6 border rounded-lg bg-white shadow-sm space-y-4">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700">Race (from roll: {initialData.race})</label>
                    <input type="text" value={race} onChange={e => setRace(e.target.value)} required placeholder="e.g., Orcs, Goblins" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
                </div>
                 <div>
                    <label className="block text-sm font-medium text-gray-700">Symbol</label>
                    <input type="text" value={symbolName} onChange={e => setSymbolName(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" disabled />
                </div>
                 <div>
                    <label className="block text-sm font-medium text-gray-700">Color</label>
                    <input type="text" value={color} onChange={e => setColor(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" disabled />
                </div>
                <div>
                    <label htmlFor="faction-name" className="block text-sm font-medium text-gray-700">Faction Name</label>
                    <input type="text" id="faction-name" value={name} onChange={e => setName(e.target.value)} required className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
                </div>
            </div>
            {!isNeighbor && (
                <div>
                    <label htmlFor="capital-name" className="block text-sm font-medium text-gray-700">Capital Name (Rule 3.1)</label>
                    <input type="text" id="capital-name" value={capitalName} onChange={e => setCapitalName(e.target.value)} required placeholder="e.g., Ironhold, Glimmerwood" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
                </div>
            )}
            <div className="relative">
                <label htmlFor="faction-emoji" className="block text-sm font-medium text-gray-700">Map Emoji</label>
                <button ref={triggerRef} type="button" id="faction-emoji" onClick={() => setIsPickerOpen(!isPickerOpen)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm text-center text-2xl bg-white" aria-haspopup="true" aria-expanded={isPickerOpen}>{emoji}</button>
                <EmojiPicker isOpen={isPickerOpen} onClose={() => setIsPickerOpen(false)} onSelect={setEmoji} triggerRef={triggerRef} />
            </div>
            <div>
                <label htmlFor="faction-theme" className="block text-sm font-medium text-gray-700">Theme</label>
                <input type="text" id="faction-theme" value={theme} onChange={e => setTheme(e.target.value)} required placeholder="e.g., Nomadic horse-riders, secretive forest-dwellers" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
            </div>
            <div>
                <label htmlFor="faction-description" className="block text-sm font-medium text-gray-700">Description</label>
                <textarea id="faction-description" value={description} onChange={e => setDescription(e.target.value)} required rows={3} placeholder="A short paragraph about the faction's culture, values, or general outlook." className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
            </div>
            {!isNeighbor && (
                <div>
                    <label htmlFor="faction-leader" className="block text-sm font-medium text-gray-700">First Leader</label>
                    <input type="text" id="faction-leader" value={leaderName} onChange={e => setLeaderName(e.target.value)} required placeholder="Name a hero to lead your people" className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-amber-500 bg-white text-gray-900" />
                </div>
            )}
            
            <div className="flex gap-4 pt-4 border-t">
                <button type="button" onClick={handleClearForm} className="w-1/3 bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors">Clear Form</button>
                <button type="submit" className="w-2/3 bg-amber-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-amber-700 transition-all transform hover:scale-105 shadow-lg">
