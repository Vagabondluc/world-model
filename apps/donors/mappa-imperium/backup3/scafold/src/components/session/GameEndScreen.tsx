...
import type { GameSettings, Faction } from '../../types';

// Copied from GameSetup for consistency
...
const GameEndScreen = () => {
    const { elements, players, gameSettings, handleExport, handleExportChronicleFeed, handleLogOff } = useGame();
    ...
    if (!stats) {
        return (
            <div className="layout-centered-card">
                <div className="page-card text-center">
                    ...
                    <p className="mt-4 text-gray-600">Could not load game summary. Game settings are missing.</p>
                     <button onClick={handleLogOff} className="mt-8 btn btn-primary">
                        Return to Main Menu
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="layout-centered-card p-4">
            <div className="page-card max-w-2xl">
                     ...
                     <div>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
                            {Object.entries(stats.elementCounts).map(([type, count]) => (
                                <span key={type} className="bg-amber-100 px-2 py-1 rounded-full">{type}: <strong>{count}</strong></span>
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 space-y-4">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <button onClick={handleExport} className="w-full btn btn-primary bg-green-700 hover:bg-green-600">
                            Export Final World (.json)
                        </button>
                        <button onClick={handleExportChronicleFeed} className="w-full btn btn-primary bg-sky-700 hover:bg-sky-600">
                            Publish Final Feed (.json)
                        </button>
                     </div>
                     <button onClick={handleLogOff} className="w-full btn btn-secondary">
                        Return to Main Menu
                    </button>
                </div>
            </div>
