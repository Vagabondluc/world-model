...
-   **`.env.local`**: (Not in version control) Stores local environment variables. Populate `GEMINI_API_KEY` to enable Gemini-powered features in local builds.
...
-   **`file_check_list.txt`**: A text file containing a complete list of expected project files, used for auditing and verifying the completeness of this documentation.
-   **`index.html`**: The main entry point for the web application. It sets up the HTML document structure, loads the Tailwind CSS CDN, defines an import map so AI Studio can resolve React from `aistudiocdn.com`, and embeds the component class library inside a large `<style>` block that leans on CSS custom properties for theming.
