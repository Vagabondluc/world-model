...
const DeityCard = ({ element, onEdit, onDelete, onExportHtml, onExportMarkdown }: DeityCardProps) => {
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const menuRef = useRef<HTMLDivElement>(null);
    useOnClickOutside(menuRef, () => setIsMenuOpen(false));

    if (element.type !== 'Deity') return null;
    const deity = element.data as Deity;

    const buttonId = `menu-button-deity-${element.id}`;
    
    return (
        <div className="bg-white p-4 rounded-lg shadow-sm border border-l-4 border-purple-500 flex flex-col gap-4">
            <div className="flex gap-4 items-start">
                ...
                <div className="flex-grow">
                     <div className="flex justify-between items-start">
                        <h4 className="text-xl font-bold text-gray-800">{deity.name}</h4>
                        <div className="relative" ref={menuRef}>
                            <button id={buttonId} onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-1 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-full transition-colors" aria-haspopup="true" aria-expanded={isMenuOpen} aria-controls={`menu-deity-${element.id}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" /></svg>
                            </button>
                            {isMenuOpen && (
                                <div id={`menu-deity-${element.id}`} className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border z-10 animate-fade-in-down" role="menu" aria-orientation="vertical" aria-labelledby={buttonId}>
                    ...
                    <span className="text-sm font-semibold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-full inline-block">
                        {deity.domain}
                    </span>
                     <p className="mt-2 text-gray-500 italic text-sm">
                        <strong>Symbol:</strong> {deity.symbol}
                     </p>
                    <p className="mt-1 text-gray-600 whitespace-pre-wrap text-sm">{deity.description}</p>
                </div>
            </div>
        </div>
    );
