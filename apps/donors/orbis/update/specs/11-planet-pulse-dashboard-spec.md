# Planet Pulse Dashboard Spec

## 1) Purpose
This dashboard provides a focused read model for `planet_pulse` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type planet_pulseViewId = "planet_pulse"
...
interface planet_pulseViewState {
  viewId: planet_pulseViewId
  snapshot: WorldSummarySnapshot
...
## 3) Core Widgets
- world status badges, trend lines, top pressures
...
### Primary Metrics (v1)
- `global_temp_k (K)`
- `active_risk_flags_count (count)`
- `biomass_index_ppm (ppm)`

### Primary Actions (v1)
- `open_story_director` (maxClicks: <= 3)
- `open_risk_details` (maxClicks: <= 3)
- `run_projection_50y` (maxClicks: <= 3)
## 7) Dependencies
