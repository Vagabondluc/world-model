# Atmosphere Console Spec

## 1) Purpose
This dashboard provides a focused read model for `atmosphere_console` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type atmosphere_consoleViewId = "atmosphere_console"

interface AtmosphereStateSnapshot {
  worldId: string
  tick: number
...
interface atmosphere_consoleViewState {
  viewId: atmosphere_consoleViewId
  snapshot: AtmosphereStateSnapshot
...
## 3) Core Widgets
- gas composition bars, breathable indicator, compatibility matrix
...
```ts
type atmosphere_consoleCommand =
  | "filter_species_by_breathability"
  | "project_co2_path"
  | "open_event_forge"
```
...
- `co2_ppm (ppm)`
- `o2_ppm (ppm)`
- `breathability_index_ppm (ppm)`

### Primary Actions (v1)
- `filter_species_by_breathability` (maxClicks: <= 3)
- `project_co2_path` (maxClicks: <= 3)
- `open_event_forge` (maxClicks: <= 3)
## 7) Dependencies
