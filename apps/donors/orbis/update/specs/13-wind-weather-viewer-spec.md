# Wind & Weather Pattern Viewer Spec

## 1) Purpose
This dashboard provides a focused read model for `wind_weather_viewer` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type wind_weather_viewerViewId = "wind_weather_viewer"

interface WindFieldSnapshot {
  worldId: string
  tick: number
...
interface wind_weather_viewerViewState {
  viewId: wind_weather_viewerViewId
...
## 3) Core Widgets
- wind vectors, storm lanes, seasonal overlays
...
```ts
type wind_weather_viewerCommand =
  | "toggle_season"
  | "inspect_cell_wind"
  | "project_drought_corridor"
```
...
### Primary Metrics (v1)
- `mean_wind_speed_mps (m/s)`
- `storm_corridor_count (count)`
- `drought_risk_ppm (ppm)`

### Primary Actions (v1)
- `toggle_season` (maxClicks: <= 3)
- `inspect_cell_wind` (maxClicks: <= 3)
- `project_drought_corridor` (maxClicks: <= 3)
## 7) Dependencies
