# Ocean & Currents Viewer Spec

## 1) Purpose
This dashboard provides a focused read model for `ocean_currents_viewer` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type ocean_currents_viewerViewId = "ocean_currents_viewer"

interface OceanCurrentSnapshot {
  worldId: string
  tick: number
...
interface ocean_currents_viewerViewState {
  viewId: ocean_currents_viewerViewId
  snapshot: OceanCurrentSnapshot
...
## 3) Core Widgets
- current vectors, upwelling zones, marine productivity
...
```ts
type ocean_currents_viewerCommand =
  | "inspect_current_band"
  | "show_anoxia_risk"
  | "forecast_coastal_impacts"
```
...
### Primary Metrics (v1)
- `sea_surface_temp_k (K)`
- `current_intensity_ppm (ppm)`
- `anoxia_risk_ppm (ppm)`

### Primary Actions (v1)
- `inspect_current_band` (maxClicks: <= 3)
- `show_anoxia_risk` (maxClicks: <= 3)
- `forecast_coastal_impacts` (maxClicks: <= 3)
## 7) Dependencies
