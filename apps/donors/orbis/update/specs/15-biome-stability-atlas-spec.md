# Biome Stability Atlas Spec

## 1) Purpose
This dashboard provides a focused read model for `biome_stability_atlas` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type biome_stability_atlasViewId = "biome_stability_atlas"

interface BiomeStabilitySnapshot {
  worldId: string
  tick: number
...
interface biome_stability_atlasViewState {
  viewId: biome_stability_atlasViewId
  snapshot: BiomeStabilitySnapshot
...
## 3) Core Widgets
- stability heatmap, transition fronts, refugia overlay
...
```ts
type biome_stability_atlasCommand =
  | "inspect_transition_risk"
  | "list_refugia"
  | "queue_mitigation_event"
```
...
### Primary Metrics (v1)
- `biome_stability_ppm (ppm)`
- `transition_front_count (count)`
- `refugia_area_km2 (km2)`

### Primary Actions (v1)
- `inspect_transition_risk` (maxClicks: <= 3)
- `list_refugia` (maxClicks: <= 3)
- `queue_mitigation_event` (maxClicks: <= 3)
## 7) Dependencies
