...
## 1) Purpose
This dashboard provides a focused read model for `species_viewer` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type species_viewerViewId = "species_viewer"

interface SpeciesProfileSnapshot {
  worldId: string
  tick: number
...
interface species_viewerViewState {
  viewId: species_viewerViewId
  snapshot: SpeciesProfileSnapshot
...
## 3) Core Widgets
- lineage graph, survivability radar, extinction risk band
...
```ts
type species_viewerCommand =
  | "open_species_sheet"
  | "compare_species"
  | "export_species_packet"
```
...
### Primary Metrics (v1)
- `species_population_ppm (ppm)`
- `habitat_fit_ppm (ppm)`
- `extinction_risk_ppm (ppm)`

### Primary Actions (v1)
- `open_species_sheet` (maxClicks: <= 3)
- `compare_species` (maxClicks: <= 3)
- `export_species_packet` (maxClicks: <= 3)
## 7) Dependencies
