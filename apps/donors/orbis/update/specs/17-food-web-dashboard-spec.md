# Food Web / Trophic Dashboard Spec

## 1) Purpose
This dashboard provides a focused read model for `food_web_dashboard` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type food_web_dashboardViewId = "food_web_dashboard"

interface TrophicNetworkSnapshot {
  worldId: string
  tick: number
...
interface food_web_dashboardViewState {
  viewId: food_web_dashboardViewId
  snapshot: TrophicNetworkSnapshot
...
## 3) Core Widgets
- trophic pyramid, bottlenecks, cascade risk
...
```ts
type food_web_dashboardCommand =
  | "inspect_trophic_edge"
  | "simulate_shock"
  | "create_ecology_hook"
```
...
### Primary Metrics (v1)
- `trophic_balance_ppm (ppm)`
- `cascade_risk_ppm (ppm)`
- `bottleneck_count (count)`

### Primary Actions (v1)
- `inspect_trophic_edge` (maxClicks: <= 3)
- `simulate_shock` (maxClicks: <= 3)
- `create_ecology_hook` (maxClicks: <= 3)
## 7) Dependencies
