# Invasive Species & Disease Watch Spec

## 1) Purpose
This dashboard provides a focused read model for `invasive_disease_watch` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type invasive_disease_watchViewId = "invasive_disease_watch"

interface BioHazardSnapshot {
  worldId: string
  tick: number
...
interface invasive_disease_watchViewState {
  viewId: invasive_disease_watchViewId
  snapshot: BioHazardSnapshot
...
## 3) Core Widgets
- spread map, host vulnerability, ETA bands
...
```ts
type invasive_disease_watchCommand =
  | "queue_quarantine_policy"
  | "trigger_response_event"
  | "inspect_spread_path"
```
...
### Primary Metrics (v1)
- `spread_front_cells (count)`
- `host_vulnerability_ppm (ppm)`
- `outbreak_eta_ticks (ticks)`

### Primary Actions (v1)
- `queue_quarantine_policy` (maxClicks: <= 3)
- `trigger_response_event` (maxClicks: <= 3)
- `inspect_spread_path` (maxClicks: <= 3)
## 7) Dependencies
