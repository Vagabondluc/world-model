# Civilization Pulse Spec

## 1) Purpose
This dashboard provides a focused read model for `civilization_pulse` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type civilization_pulseViewId = "civilization_pulse"
...
interface civilization_pulseViewState {
  viewId: civilization_pulseViewId
  snapshot: CivilizationProfileSnapshot
...
## 3) Core Widgets
- faction scorecards, unrest forecast, doctrine badges
...
```ts
type civilization_pulseCommand =
  | "open_faction_card"
...
### Primary Metrics (v1)
- `governance_stability_ppm (ppm)`
- `unrest_risk_ppm (ppm)`
- `war_risk_ppm (ppm)`

### Primary Actions (v1)
- `open_faction_card` (maxClicks: <= 3)
- `simulate_policy_shift` (maxClicks: <= 3)
- `open_conflict_board` (maxClicks: <= 3)
## 7) Dependencies
