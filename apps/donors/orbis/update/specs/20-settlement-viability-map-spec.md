# Settlement Viability Map Spec

## 1) Purpose
This dashboard provides a focused read model for `settlement_viability_map` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type settlement_viability_mapViewId = "settlement_viability_map"
...
interface settlement_viability_mapViewState {
  viewId: settlement_viability_mapViewId
  snapshot: SettlementViabilitySnapshot
...
## 3) Core Widgets
- suitability heatmap, resource slices, relocation candidates
...
```ts
type settlement_viability_mapCommand =
  | "inspect_site"
  | "compare_sites"
  | "queue_settlement_shift"
```
...
### Primary Metrics (v1)
- `viability_score_ppm (ppm)`
- `water_stress_ppm (ppm)`
- `relocation_risk_ppm (ppm)`

### Primary Actions (v1)
- `inspect_site` (maxClicks: <= 3)
- `compare_sites` (maxClicks: <= 3)
- `queue_settlement_shift` (maxClicks: <= 3)
## 7) Dependencies
