# Trade & Supply Lanes Spec

## 1) Purpose
This dashboard provides a focused read model for `trade_supply_lanes` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type trade_supply_lanesViewId = "trade_supply_lanes"
...
interface trade_supply_lanesViewState {
  viewId: trade_supply_lanesViewId
...
## 3) Core Widgets
- route graph, chokepoint alerts, disruption impact
...
```ts
type trade_supply_lanesCommand =
  | "inspect_route"
  | "simulate_route_failure"
  | "queue_trade_event"
```
...
### Primary Metrics (v1)
- `route_capacity_ppm (ppm)`
- `chokepoint_count (count)`
- `supply_shock_risk_ppm (ppm)`

### Primary Actions (v1)
- `inspect_route` (maxClicks: <= 3)
- `simulate_route_failure` (maxClicks: <= 3)
- `queue_trade_event` (maxClicks: <= 3)
## 7) Dependencies
