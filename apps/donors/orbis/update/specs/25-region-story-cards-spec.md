# Region Story Cards Spec

## 1) Purpose
This dashboard provides a focused read model for `region_story_cards` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type region_story_cardsViewId = "region_story_cards"
...
interface region_story_cardsViewState {
  viewId: region_story_cardsViewId
  snapshot: RegionStorySnapshot
...
## 3) Core Widgets
- pressure stack, likely next events, hook generator
...
```ts
type region_story_cardsCommand =
  | "generate_session_hooks"
  | "generate_chapter_hooks"
  | "pin_canon_entry"
```
...
### Primary Metrics (v1)
- `regional_pressure_ppm (ppm)`
- `hook_readiness_ppm (ppm)`
- `next_event_probability_ppm (ppm)`

### Primary Actions (v1)
- `generate_session_hooks` (maxClicks: <= 3)
- `generate_chapter_hooks` (maxClicks: <= 3)
- `pin_canon_entry` (maxClicks: <= 3)
## 7) Dependencies
