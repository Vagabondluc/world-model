# Determinism & Replay Integrity Spec

## 1) Purpose
This dashboard provides a focused read model for `determinism_replay_integrity` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type determinism_replay_integrityViewId = "determinism_replay_integrity"
...
interface determinism_replay_integrityViewState {
  viewId: determinism_replay_integrityViewId
  snapshot: ReplayIntegritySnapshot
...
## 3) Core Widgets
- digest parity, mismatch timeline, replay confidence
...
```ts
type determinism_replay_integrityCommand =
  | "inspect_digest_mismatch"
...
### Primary Metrics (v1)
- `digest_mismatch_count (count)`
- `replay_confidence_ppm (ppm)`
- `parity_score_ppm (ppm)`

### Primary Actions (v1)
- `inspect_digest_mismatch` (maxClicks: <= 3)
- `export_replay_packet` (maxClicks: <= 3)
- `rerun_seed_check` (maxClicks: <= 3)
## 7) Dependencies
