# Parameter Provenance Explorer Spec

## 1) Purpose
This dashboard provides a focused read model for `parameter_provenance_explorer` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type parameter_provenance_explorerViewId = "parameter_provenance_explorer"
...
interface parameter_provenance_explorerViewState {
  viewId: parameter_provenance_explorerViewId
  snapshot: ParameterProvenanceSnapshot
...
## 3) Core Widgets
- source badges, change history, confidence markers
...
```ts
type parameter_provenance_explorerCommand =
  | "inspect_parameter_lineage"
  | "compare_versions"
  | "export_audit_packet"
```
...
### Primary Metrics (v1)
- `stale_parameter_count (count)`
- `earth_sourced_share_ppm (ppm)`
- `fitted_share_ppm (ppm)`

### Primary Actions (v1)
- `inspect_parameter_lineage` (maxClicks: <= 3)
- `compare_versions` (maxClicks: <= 3)
- `export_audit_packet` (maxClicks: <= 3)
## 7) Dependencies
