...
## 1) Purpose
This dashboard provides a focused read model for `tag_explorer` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type tag_explorerViewId = "tag_explorer"
...
interface tag_explorerViewState {
  viewId: tag_explorerViewId
  snapshot: TagPropagationSnapshot
...
## 3) Core Widgets
- active tags, intensity timelines, overlap warnings
...
- `overlap_warning_count (count)`
- `tag_intensity_ppm (ppm)`

### Primary Actions (v1)
- `inspect_tag_origin` (maxClicks: <= 3)
- `compare_tag_sets` (maxClicks: <= 3)
- `open_registry_owner` (maxClicks: <= 3)
## 7) Dependencies
