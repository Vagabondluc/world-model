# World Compare (A/B) Spec

## 1) Purpose
This dashboard provides a focused read model for `world_compare_ab` and exposes high-leverage actions without requiring raw solver tuning.

## 2) View Contract

```ts
type world_compare_abViewId = "world_compare_ab"
...
interface world_compare_abViewState {
  viewId: world_compare_abViewId
  snapshot: WorldComparisonSnapshot
...
## 3) Core Widgets
- side-by-side deltas, map diff slider, commit recommendation
...
```ts
type world_compare_abCommand =
  | "select_baseline"
  | "apply_overlay"
  | "commit_branch_choice"
```
...
### Primary Metrics (v1)
- `delta_score_ppm (ppm)`
- `map_divergence_ppm (ppm)`
- `commit_advice_confidence_ppm (ppm)`

### Primary Actions (v1)
- `select_baseline` (maxClicks: <= 3)
- `apply_overlay` (maxClicks: <= 3)
- `commit_branch_choice` (maxClicks: <= 3)
## 7) Dependencies
