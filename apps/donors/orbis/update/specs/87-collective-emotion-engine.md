# 🔒 COLLECTIVE EMOTION ENGINE SPEC v1 (FROZEN)

SpecTier: Contract

## Spec Header
- `Version`: `v1`
- `DependsOn`: [`docs/specs/00-core-foundation/02-metric-registry.md`, `docs/specs/00-core-foundation/03-threshold-registry.md`, `docs/specs/70-governance-benchmarks/84-legitimacy-and-authority-engine.md`, `docs/specs/70-governance-benchmarks/86-information-control-and-media-engine.md`, `docs/specs/30-runtime-determinism/60-event-schema-reason-code-registry.md`]
- `Owns`: [`EmotionPopulationBlockIdV1`, `CollectiveEmotionInputsV1`, `EmotionStateV1`, `CollectiveEmotionStateV1`, `EmotionInstanceV1`]
- `Writes`: [`emotion state`, `emotion instances`]

## Canonical Owner
- `CANONICAL_OWNER`: `docs/specs/70-governance-benchmarks/87-collective-emotion-engine.md`
- `STATUS`: `FROZEN`
...
## Non-Ownership
- This file owns emotional modulation state and contagion behavior.
- It does not own metric bounds or threshold rule definitions (`02`, `03`).
...
```ts
type EmotionPopulationBlockIdV1 = string

interface CollectiveEmotionInputsV1 {
  populationBlockId: EmotionPopulationBlockIdV1
  fearPPM: PpmInt
  hopePPM: PpmInt
  angerPPM: PpmInt
