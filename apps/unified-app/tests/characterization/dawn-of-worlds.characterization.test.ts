...
const captureRoot = path.join(repoRoot, "tests", "characterization", "dawn-of-worlds", "captured");

function readJson(name: string) {
  return JSON.parse(fs.readFileSync(path.join(captureRoot, name), "utf8"));
}

describe("dawn of worlds characterization", () => {
  it("captures app-donor metadata and panels", () => {
    ...
    expect(uiBaseline.panels).toEqual(expect.arrayContaining(["World kind", "Turn controls", "Multiplayer session"]));
  });
});
