...
const captureRoot = path.join(repoRoot, "tests", "characterization", "mappa-imperium", "captured");

function readJson(name: string) {
  return JSON.parse(fs.readFileSync(path.join(captureRoot, name), "utf8"));
}

describe("mappa imperium characterization", () => {
  it("captures app-donor metadata and panels", () => {
    ...
    expect(uiBaseline.panels).toEqual(expect.arrayContaining(["Era timeline", "Territory map", "Session controls"]));
  });
});
