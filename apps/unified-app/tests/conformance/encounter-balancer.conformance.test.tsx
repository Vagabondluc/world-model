...
describe("encounter balancer conformance", () => {
  it("mounts the donor route through the non-exact representative scaffold", () => {
    const definition = DONOR_DEFINITIONS["encounter-balancer"];
    renderAt(definition.route);

    const host = screen.getByTestId("donor-subapp-host");

    expect(screen.getByRole("heading", { name: "Encounter Balancer Scaffold" })).toBeInTheDocument();
    expect(host).toHaveAttribute("data-donor-id", "encounter-balancer");
