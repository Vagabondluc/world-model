...
describe("faction image conformance", () => {
  it("mounts the donor route through the non-exact subapp scaffold", () => {
    const definition = DONOR_DEFINITIONS["faction-image"];
    renderAt(definition.route);

    const host = screen.getByTestId("donor-subapp-host");

    expect(screen.getByRole("heading", { name: "Sacred Sigil Generator" })).toBeInTheDocument();
    expect(host).toHaveAttribute("data-donor-id", "faction-image");
